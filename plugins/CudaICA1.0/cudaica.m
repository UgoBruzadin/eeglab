...
if contains(pwd,' ')
    error('CUDAICA cannot run when the current path have space character!');
end

% import CUDAICABINARY and SC
icadefs;
clearvars -except data varargin CUDAICABINARY SC
...
if ischar(data)
    ...
    if nargin < 3
        error('Not enough input arguments! Data file name must be followed by nchans, nframes.');
    end
    nchans = varargin{1};
    nframes = varargin{2};
    varargin(1:2) = [];
    if ischar(nchans) || ischar(nframes)
        error('nchans, nframes arguments must be given as numbers\n');
    end
else
    % Data array provided.
    if size(data,3) > 1
...
if ~exist(scriptfile,'file')
    error('ICA script file "%s" not written.\n',scriptfile);
end
...
if ~exist(datafile,'file')
    error('ICA data file "%s" not written.\n',datafile);
end
