% pop_eegplot_w2() - Visually inspect EEG data using a scrolling display.
...
%
% Usage:
%   >> pop_eegplot_w2( EEG ) % Scroll EEG channel data. Allow marking for rejection via
...
%   >> pop_eegplot_w2( EEG, icacomp, superpose, reject );
%
% Graphic interface:
...
function com = pop_eegplot_w2_list( EEG, icacomp, superpose, reject, topcommand, channels, varargin)
...
elseif ~isempty(EEG.chanlocs) && icacomp == 2
    eegplotoptions = { eegplotoptions{:}  'eloc_file', EEG.chanlocs(channels) };
else
    if ~icacomp 
...
elseif icacomp == 2
    ...
    eegplot_w( EEG.data(channels,:,:), 'srate', EEG.srate, 'title', [ 'Scroll CHOSEN channel activities -- eegplot_w2_list(): ' EEG.setname], ...
			  'limits', [EEG.xmin EEG.xmax]*1000 , 'command', command, eegplotoptions{:}, varargin{:});
elseif icacomp == 3
