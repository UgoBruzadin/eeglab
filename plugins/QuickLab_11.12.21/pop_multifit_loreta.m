    ...
    %for i = comps(:)'
        %if i <= length(EEG.dipfit.model) && ~isempty(EEG.dipfit.model(i).posxyz)
            if g.dipoles == 2
                ...
                EEG.loreta.model.active = [1 2];
                EEG.loreta.model.select = [1 2];
                if isempty(EEG.loreta.model.posxyz)
                    EEG.loreta.model.posxyz = zeros(1,3);
                    EEG.loreta.model.momxyz = zeros(2,3);
                else
                    EEG.loreta.model.posxyz(2,:) = EEG.loreta.model.posxyz;
                    if strcmpi(EEG.loreta.coordformat, 'MNI')
                         EEG.loreta.model.posxyz(:,1) = [-40;40];
                    else EEG.loreta.model.posxyz(:,2) = [-40;40];
                    end
                    EEG.loreta.model.momxyz(2,:) = EEG.loreta.model.momxyz;
                end
            else 
%                 EEG.loreta.model.active = 1;
%                 EEG.loreta.model.select = 1;
            end
            ...
            %try
                ...
                %else
                    EEG = dipfit_nonlinear_loreta(EEG, 'component', i, 'symmetry', []);
                %end
            %catch, EEG.dipfit.model.rv = NaN; disp('Maximum number of iterations reached. Fitting failed');
            %end
