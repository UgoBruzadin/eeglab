% eegplot_w2freq() - Scroll (horizontally and/or vertically) through multichannel data.
...
%
% Usage: 
%           >> eegplot_w2freq(data, 'key1', value1 ...); % use interface buttons, etc.
%      else
%           >> eegplot_w2freq('noui', data, 'key1', value1 ...); % no user interface;
%                                                         % use for plotting
% Menu items:
...
%    'tag'        - [string] Matlab object tag to identify this eegplot_w2freq() window (allows 
%                    keeping track of several simultaneous eegplot_w2freq() windows). 
%    'children'   - [integer] Figure handle of a *dependent* eegplot_w2freq() window. Scrolling
...
function [outvar1,EEG] = eegplot_w2freq(EEG, varargin); % p1,p2,p3,p4,p5,p6,p7,p8,p9)
...
if isstruct(EEG)
    ...
    if ~isfield(EEG,'freq')
        ...
        [data,freqs] = spectopo(EEG.data(:,:),EEG.pnts,EEG.srate,'plot','off','winsize',maxWindow);
        EEG.tmpcolors = rand(size(data,1),3);
        EEG.freq = data;
        g.isfreq = 1;
        g.freqs = [0:55];
        g.freqlimits = [0;55];
...
if ~ischar(data) % If NOT a 'noui' call or a callback from uicontrols
   ...
   try
       ...
       disp('eegplot_w2freq() error: calling convention {''key'', value, ... } error'); return;
   end;	
   if isstruct(EEG)
       g.EEG = EEG;
   end
...
%        % Check  consistency of freqs
  ...
  % ---------------------------------
  defdowncom   = 'eegplot_w2freq(''defdowncom'',   gcbf);'; % push button: create/remove window
  defmotioncom = 'eegplot_w2freq(''defmotioncom'', gcbf);'; % motion button: move windows or display current position
  defupcom     = 'eegplot_w2freq(''defupcom'',     gcbf);';
  defctrldowncom = 'eegplot_w2freq(''topoplot'',   gcbf);'; % CTRL press and motion -> do nothing by default
  ...
  defctrlupcom = ''; % CTRL press and up -> do nothing by default
   ...
   try, g.spacing; 			catch, g.spacing	= .3; 	end;
   try, g.eloc_file; 		catch, g.eloc_file	= 0; 	end; % 0 mean numbered
   try, g.winlength; 		catch, g.winlength	= .4; 	end; % Number of seconds of EEG displayed
   try, g.fullscreen; 	    catch, g.fullscreen = 'on';	end;
   try, g.position; 	    catch, g.position	= ORIGINAL_POSITION; g.fullscreen = 'on';	end;
   try, g.title; 		    catch, g.title		= ['Scroll activity -- eegplot_w2freq()']; 	end;
   ...
   try, g.tag; 				catch, g.tag		= 'eegplot_w2freq'; end;
   try, g.xgrid;		    catch, g.xgrid		= 'off'; end;
   try, g.ygrid;		    catch, g.ygrid		= 'off'; end;
   try, g.color;		    catch, g.color		= 'on'; end;
   ...
   try, g.freqlimits;	    catch, g.freqlimits	= [2 55]; end;
   ...
   for index=1:length(gfields)
      ...
      case {'spacing', 'srate' 'eloc_file' 'winlength' 'fullscreen' 'position' 'title' 'plottitle' ...
               ...
               'mocap' 'selectcommand' 'ctrlselectcommand' 'datastd' 'normed' 'envelope' 'isfreq' 'EEG' 'rndcolors' 'savecommand'}
      otherwise, error(['eegplot_w2freq: unrecognized option: ''' gfields{index} '''' ]);
      end;
   end;
  ...
  if ischar(g.eloc_file) || isstruct(g.eloc_file)  % Read in electrode names
      ...
      eegplot_w2freq('setelect', g.eloc_file, ax1);
  end;
...
% positions of buttons
  posbut(22,:) = [ 0.92    0.94    0.080    0.02 ]; % stack channels(same offset)  
  posbut(21,:) = [ 0.92    0.92    0.080    0.02 ]; % normalize
  posbut(17,:) = [ 0.92    0.90    0.080    0.02 ]; % events types
  
  posbut(14,:) = [ 0.92    0.86    0.080    0.02 ]; % elec tag
  posbut(7,:) =  [ 0.92    0.84    0.080    0.02 ]; % elec
  
  posbut(16,:) = [ 0.92    0.82    0.080    0.02 ]; % value tag
  posbut(9,:) =  [ 0.92    0.80    0.080    0.02 ]; % value
  
  posbut(15,:) = [ 0.92    0.78    0.080    0.02 ]; % g.time tag
  posbut(8,:) =  [ 0.92    0.76    0.080    0.02 ]; % g.time   
  
  posbut(5,:) =  [ 0.92    0.62    0.080    0.02 ]; % Eposition .52
  posbut(1,:) =  [ 0.92    0.60    0.020    0.02 ]; % <<  
  posbut(2,:) =  [ 0.94    0.60    0.020    0.02 ]; % <  
  posbut(3,:) =  [ 0.96    0.60    0.020    0.02 ]; % >
  posbut(4,:) =  [ 0.98    0.60    0.020    0.02 ]; % >>  
  posbut(23,:) = [ 0.92    0.58    0.080    0.02 ]; % Espacing/scale tag
  posbut(6,:) =  [ 0.92    0.56    0.080    0.02 ]; % Espacing/scale  
  posbut(10,:) = [ 0.96    0.54    0.020    0.02 ]; % +  
  posbut(11,:) = [ 0.94    0.54    0.020    0.02 ]; % -  
...
%   posbut(36,:) = [ 0.94    0.54    0.020    0.02 ]; % - 
  ...
  posbut(27,:) = [ 0.92    0.18    0.080    0.03 ]; % +  
  posbut(28,:) = [ 0.92    0.21    0.080    0.03 ]; % -  
...
displayep = ['eegplot_w2freq(''redraw'')'];
togglerej = ['eegplot_w2freq(''rejection'')'];
displaycomp = ['EEG.plotEp = 1 - EEG.plotEp; eegplot_w2freq(EEG,varargin)'];
...
% 	'FontSize',8,...
% 	'Tag','thechannel',...
% 	'string','PARTIAL interp, use with ; or ,');
...
cb_envelope = ['g = get(gcbf,''userdata'');'...
    ...
    'eegplot_w2freq(''drawp'',0);clear g;'];
...
u(22) = uicontrol('Parent',figh, ...
  ...
  if ~isempty(g.events)
      u(17) = uicontrol('Parent',figh, ...
                        'Units', 'normalized', ...
                        'Position',posbut(17,:), ...
                        'string', 'Events', 'callback', 'eegplot_w2freq(''drawlegend'', gcbf)');
  end;
  ...
  uimenu('Parent',m(7),'Label','Edit figure', 'Callback', 'eegplot_w2freq(''noui'');');
  ...
  % Scale Eye %%%%%%%%%
  timestring = ['[OBJ1,FIG1] = gcbo;',...
	        'eegplot_w2freq(''scaleeye'',OBJ1,FIG1);',...
		'clear OBJ1 FIG1;'];
  m(7) = uimenu('Parent',m(1),'Label','Show scale','Callback',timestring);
  
  % Title %%%%%%%%%%%%
  uimenu('Parent',m(1),'Label','Title','Callback','eegplot_w2freq(''title'')')
  ...
  uimenu('Parent',m(2),'Label','Time range to display',...
      'Callback','eegplot_w2freq(''window'')')
  
  % Electrode window %%%%%%%%
  uimenu('Parent',m(2),'Label','Number of channels to display',...
      'Callback','eegplot_w2freq(''winelec'')')
  ...
  uimenu('Parent',m(6),'Label','Load .loc(s) file',...
      'Callback','eegplot_w2freq(''loadelect'');')
 ...
 % end
  ...
  uimenu('Parent',figh,'Label', 'Help', 'callback', 'pophelp(''eegplot_w2freq'');');

  % Events %%%%%%%%
  ev = uimenu('Parent',m(2),'Label','Events');
  comeventmaxstring   = [ 'tmpg = get(gcbf, ''userdata'');' ...
                'tmpg.plotevent = ''on'';' ...
                'set(gcbf, ''userdata'', tmpg); clear tmpg; eegplot_w2freq(''emaxstring'');']; % JavierLC      
  comeventleg  = [ 'eegplot_w2freq(''drawlegend'', gcbf);'];
...
%  set(figh, 'windowbuttonupfcn', commandrelease);
  ...
  eegplot_w2freq('scaleeye', [], gcf);
  if strcmp(lower(g.scale), 'off')
	  eegplot_w2freq('scaleeye', 'off', gcf);
  end;
  
  eegplot_w2freq('drawp', 0);
  
  if g.dispchans ~= g.chans
  	   eegplot_w2freq('zoom', gcf);
  end;  
  eegplot_w2freq('scaleeye', [], gcf);
  ...
  set(h, 'backgroundcolor', [0.7 1 0.7]);
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  % make sure data selection & rejection is not affected.
  ...
  case 'rejection'
   ...
   case 'redraw'
    ...
    if g.trialstag == -1
        ...
        set(dis,'BackgroundColor',[1 0.5 0.5]); %make display blue
...
{ 'Number of channels to display:' } , 'Change number of channels to display', 1,  { num2str(g.dispchans) });
   ...
   eegplot_w2freq('updateslider', fig);
   eegplot_w2freq('drawp',0);	
   eegplot_w2freq('scaleeye', [], fig);
   %MarkChannel([],[],fig,0,0);
   %Fixing UGO
   %set(gcf,'UserData',g);
   %draw_data([],[],fig,0,[],g);
  ...
  case 'loadelect' % load channels
	...
	eegplot_w2freq('setelect',[ inputpath inputname ],AXH0);
	return;
  ...
  case 'noui'
      if ~isempty(varargin)
          eegplot_w2freq( varargin{:} ); fig = gcf;
      else 
          fig = findobj('tag', 'eegplot_w2freq');
      end;
...
%     end;
  ...
  case 'defdowncom'
   ...
   otherwise
      error(['Error - invalid eegplot_w2freq() parameter: ',data])
  end
...
function draw_data(varargin)
    ...
    % channels in the eegplot_w2freq figure window.
    ...
    if ~isempty(g.winrej)
        ...
        for j =1:length(tmp_plot_data_y)
            plot(ax1,tmp_plot_data_y', 'color', rand(1,3), 'clipping','on');
        end
            % --- MAKE NEW LIMITS
        highlim2 = highlim;
        lowlim2 = lowlim;
...
if strcmpi(g.plotevent, 'on')
    ...
    AXES_POSITION = [0.05 0.03 0.865 0.96]; % UGO here's where change the heigth of the graph!
end;
...
switch evnt.Key
    ...
    case {'insert'} %CHANGED UGO
        eegplot_w2freq('window');
    case {'tab'}
        eegplot_w2freq('winelec');
    case {'v'}
        draw_data([],[],fig,0,[],[],[],...
...
%     end;
    ...
    if point_is_valid
        ...
        topoplot(data(:,datapos), EEG.chanlocs);
    end
    else
%     ax1 = findobj('tag','backeeg','parent',fig); 
%     tmppos = get(ax1, 'currentpoint');
