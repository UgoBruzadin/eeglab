function [fh, EEG, com] = pop_prop_extended2(EEG, typecomp, chanorcomp, winhandle, spec_opt, erp_opt, scroll_event, classifier_name, varargin)
...
% 	end;
	...
	com = sprintf('pop_prop_extended2( %s, %d, [%s], 0, %s, %s, %d, ''%s''', inputname(1), ...
                  typecomp, int2str(chanorcomp), vararg2str({spec_opt}), vararg2str({erp_opt}), scroll_event, classifier_name);
    if ~isempty(varargin)
        com = [com sprintf(', %s', vararg2str(varargin))];
    end
