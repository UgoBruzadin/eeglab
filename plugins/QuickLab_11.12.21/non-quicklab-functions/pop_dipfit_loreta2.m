...
function com = pop_dipfit_loreta2(EEG, select, varargin)
...
dataPre = eeglab2fieldtrip(EEG, 'raw', 'none');
...
cfg.tapsmofrq = 2;
cfg.foilim    = [2 55];
cfg.channelcmb = {'all' 'all'}; %added Ugo
...
%for iSelect = select(:)' %changes ugo
...
%% history
disp('Done');
com = sprintf('pop_dipfit_loreta2(EEG, %s);', vararg2str( { select }));
