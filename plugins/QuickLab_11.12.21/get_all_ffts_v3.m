...
FinalFolder_tables = cell(size(FullFolderList,1),2);
...
for j = 2: size(FullFolderList,1)
    ...
    FinalFolder_tables{j,1} = FullFileList;
    FinalFolder_tables{j,2} = FinalTable; %store in the big table

    save(['All_ffts_Names.mat'],'FinalTable'); %saves the table in .mat format
