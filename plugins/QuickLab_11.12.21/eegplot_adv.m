...
%% Epoch editbox, arrows, scale size, number of channels display
  ...
  posbut(20,:) = [ 0.004   0.02    0.010    0.96 ]; % slider  
  ...
  posbut(36,:) = [ 0.915    0.55+r    0.040    defaultsizes(1) ]; % Percent of Trials Tag
  posbut(37,:) = [ 0.955    0.55+r    0.040    defaultsizes(1) ]; % Number of Channels Tag
  posbut(38,:) = [ 0.915    0.53+r    0.040    defaultsizes(1) ]; % Percent of Trials box
  posbut(39,:) = [ 0.955    0.53+r    0.040    defaultsizes(1) ]; % Number of Channels Box
  
  posbut(40,:) = [ 0.915    0.51+r    0.040    defaultsizes(1) ]; % Run button % run code from box 1 and 2, add data to winrej, redraw
  
  %posbut(32,:) = [ 0.96    0.51+r    0.040    defaultsizes(1) ]; % Apply Rejections;
  
  posbut(41,:) = [ 0.955    0.51+r    0.040    defaultsizes(1) ]; % Clear marks button
...
% original = works
  ...
  posbut(54,:) = [ 0.915    0.25+r    0.040    defaultsizes(1) ]; % plot FFT
  posbut(55,:) = [ 0.955    0.25+r    0.040    defaultsizes(1) ]; % plot ICA headmaps
...
% Choose Methods tag
  u(25) = uicontrol('Parent',figh,'Units', 'normalized','Position', posbut(25,:),'Style','text','FontSize',8, ...
	'Tag','ListTag','BackgroundColor',DEFAULT_FIG_COLOR,'string','Select Functions Library');

% Methods tag
  u(32) = uicontrol('Parent',figh,'Units', 'normalized','Position', posbut(31,:),'Style','popupmenu','HorizontalAlignment', 'center','FontSize',10,...
    'Tag','ListPopup','BackgroundColor',DEFAULT_PLOT_BACKGROUND,'string',g.allmethods,'callback',@change_list);

% methods
  u(33) = uicontrol('Parent',figh,'Units', 'normalized','Position', posbut(33,:),'Style','popupmenu',...
	'Tag','ALLmethods','BackgroundColor',DEFAULT_PLOT_BACKGROUND,'string',g.tbtmethods,'callback',@change_options);

% options
  u(34) = uicontrol('Parent',figh,'Units', 'normalized','Position', posbut(34,:),'Style','edit',...
	'Tag','ALLoptions','BackgroundColor',DEFAULT_PLOT_BACKGROUND,'string',g.tbtoptions{1});

% options hints
  u(35) = uicontrol('Parent',figh,'Units', 'normalized','Position', posbut(35,:),'Style','text','FontSize',8,...
    'Tag','ALLhints','BackgroundColor',DEFAULT_FIG_COLOR,'string',g.tbtoptions{1,2});
% hint
