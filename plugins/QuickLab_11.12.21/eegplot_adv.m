% eegplot_adv() - Scroll (horizontally and/or vertically) through multichannel data.
...
%
% Usage: 
%           >> eegplot_adv(data, 'key1', value1 ...); % use interface buttons, etc.
%      else
%           >> eegplot_adv('noui', data, 'key1', value1 ...); % no user interface;
%                                                         % use for plotting
% Menu items:
...
% make_eloc_file)
  ...
  posbut(69,:) = [ 0.96    0.80    0.040    defaultsizes(1) ]; %  ICA weights/rank

  posbut(5,:) =  [ 0.92    0.78    0.080    defaultsizes(1) ]; % Eposition .52
  posbut(46,:) = [ 0.92    0.76    0.010    defaultsizes(1) ]; % |<<  % NEW UGO
  posbut(1,:) =  [ 0.93    0.76    0.015    defaultsizes(1) ]; % <<  
  posbut(2,:) =  [ 0.945   0.76    0.015    defaultsizes(1) ]; % <  
  posbut(3,:) =  [ 0.96    0.76    0.015    defaultsizes(1) ]; % >
  posbut(4,:) =  [ 0.975   0.76    0.015    defaultsizes(1) ]; % >>  
  posbut(47,:) = [ 0.99    0.76    0.010    defaultsizes(1) ]; % >>|  % NEW UGO

  posbut(23,:) = [ 0.92    0.74    0.040    defaultsizes(1) ]; % Espacing/scale tag
  posbut(6,:) =  [ 0.92    0.72    0.040    defaultsizes(1) ]; % Espacing/scale  

   posbut(48,:) = [ 0.96    0.74    0.040    defaultsizes(1) ]; % NumChan tag % NEW UGO
   posbut(49,:) = [ 0.96    0.72    0.040    defaultsizes(1) ]; % NumChan % NEW UGO

  posbut(52,:) = [ 0.96    0.70    0.040    defaultsizes(1) ]; % Reset chan numbers to max auto % NEW UGO

  posbut(10,:) = [ 0.94    0.70    0.020    defaultsizes(1) ]; % +  
  posbut(11,:) = [ 0.92    0.70    0.020    defaultsizes(1) ]; % -  
  ...
  g.headpos =     [ 0.922   0.25+r      0.075    0.13 ]; %position of topoplot
  g.matrixpos =   [ 0.92    0.25+r      0.080    0.13 ];  %position of matrix
...
% rotated  new vertical buttons
  ...
  posbut(57,:) = [ 0.92    0.17    0.080    defaultsizes(1) ]; % Folder dropmenu
  posbut(58,:) = [ 0.92    0.19    0.080    defaultsizes(1) ]; % Folder tag
  ...
  posbut(59,:) = [ 0.92    0.14    0.080    defaultsizes(1) ]; % Save text tag
  posbut(44,:) = [ 0.92    0.12    0.080    defaultsizes(1) ]; % SAVE TEXT EDIT
...
% tag
  ...
  u(40) = uicontrol('Parent',figh, ...
	...
	'Position', posbut(40,:), ...
	'Tag','TBT',...
    'BackgroundColor',DEFAULT_OFF_COLOR,...
	'string','Run',...
...
% 	'string','Tag, Save & ICA',...
% 	'Callback', ['eegplot_adv(''saveandtag'')'] );
...
%% channel or epoch, rejection or interpolation buttons #Ugo
 ...
 u(27) = uicontrol('Parent',figh, ...
	...
	'Position', posbut(27,:), ...
	'Tag','Rejection',...
    'BackgroundColor',DEFAULT_ON_COLOR,...
	'string',vert_string('INTERP MODE'),...
    'FontSize',8,...
    'FontWeight','bold',...
	'Callback', togglerej );
...
%% Text edit fields: NumChans
  ...
  u(49) = uicontrol('Parent',figh, ...
	...
	'Callback', 'eegplot_adv(''winelec_text'')' );
...
%     draw_data2(fig,[],[],tmpcolor)
  ...
  case 'rejection'
    ...
    if g.wincolor == DEFAULT_PLOT_INTERP
        ...
        set(dis,'string',vert_string('REJCT MODE'));
        ...
        set(dis,'string',vert_string('INTRP MODE'));
