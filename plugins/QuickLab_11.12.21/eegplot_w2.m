...
% positions of buttons
  ...
  posbut(29,:) = [ 0.92    0.78    0.080    defaultsizes(1) ]; % COUNTING Tag #Ugo
  posbut(30,:) = [ 0.92    0.80    0.080    defaultsizes(1) ]; % COUNTING Selected areas #Ugo
  
  posbut(5,:) =  [ 0.92    0.76    0.080    defaultsizes(1) ]; % Eposition .52
  posbut(1,:) =  [ 0.92    0.74    0.020    defaultsizes(1) ]; % <<  
  posbut(2,:) =  [ 0.94    0.74    0.020    defaultsizes(1) ]; % <  
  posbut(3,:) =  [ 0.96    0.74    0.020    defaultsizes(1) ]; % >
  posbut(4,:) =  [ 0.98    0.74    0.020    defaultsizes(1) ]; % >>  
  posbut(23,:) = [ 0.92    0.72    0.080    0.02 ]; % Espacing/scale tag
  posbut(6,:) =  [ 0.92    0.70    0.080    defaultsizes(1) ]; % Espacing/scale  
  posbut(10,:) = [ 0.96    0.68    0.020    defaultsizes(1) ]; % +  
  posbut(11,:) = [ 0.94    0.68    0.020    defaultsizes(1) ]; % -  
  ...
  posbut(35,:) = [ 0.92    0.58    0.080    defaultsizes(1) ]; % TextBox with options
  posbut(36,:) = [ 0.92    0.56    0.040    defaultsizes(1) ]; % Percent of Trials Tag
  posbut(37,:) = [ 0.96    0.56    0.040    defaultsizes(1) ]; % Number of Channels Tag
  posbut(38,:) = [ 0.92    0.54    0.040    defaultsizes(1) ]; % Percent of Trials box
  posbut(39,:) = [ 0.96    0.54    0.040    defaultsizes(1) ]; % Number of Channels Box
  posbut(40,:) = [ 0.92    0.52    0.080    defaultsizes(1) ]; % Run button % run code from box 1 and 2, add data to winrej, redraw
  posbut(41,:) = [ 0.92    0.50    0.080    defaultsizes(1) ]; % Clear marks button
  ...
  posbut(12,:) = [ 0.92    0.03    0.080    defaultsizes(3) ]; % accept/close     
...
% channel or epoch, rejection or interpolation buttons
...
% number of marked areas for control
...
%     end
    ...
    function g = TBT(g)
        ...
        switch met(1).Value
        case 1
            comrej  = ['EEG = pop_eegthresh(EEG, 1, 1:EEG.nbchan,' opt.Value ', 1, 0);'];
            bads    = 'rejthreshE';
        case 2
            comrej  = ['[EEG, comrej] = pop_rejtrend(EEG, 1, 1:EEG.nbchan,' opt.Value ', 1, 0,0);'];
            bads    = 'rejconstE';
        case 3
            comrej  = ['[EEG, ~,~,~,comrej] = pop_jointprob(EEG, 1, 1:EEG.nbchan,' opt.Value ', 1, 0, 0);'];
            bads    = 'rejjpE';
        case 4
            comrej  = ['[EEG, ~,~,~,comrej] = pop_rejkurt(EEG, 1, 1:EEG.nbchan,' opt.Value ', 1, 0, 0);'];
            bads    = 'rejkurtE';
        case 5
            comrej  = ['[EEG, ~, comrej]    = pop_rejspec(EEG, 1,' opt.Value ');'];
            bads    = 'rejfreqE';
        case 6
            comrej  = ['[EEG, comrej]    = pop_eegmaxmin(EEG,' opt.Value ');'];
            bads    = 'rejmaxminE';
        end
        
        @(EEG)comrej;
        
        winrej = EEG.reject.(bads);
%% convert bads from cell to array
if iscell(winrej)
    fprintf('pop_TBT(): Converting cell-array.')
    winrej = tbt_cell2bool(winrej,EEG);
    fprintf('.. done.\n')
end
...
channel_index           = (sum(winrej,2)/EEG.trials) >= str2double(pctbadtrial(1).String);   % boolean list
...
if sum(channel_index)
    g.eloc_file(channel_index).badchan = 1; % marks channels as bad
end
