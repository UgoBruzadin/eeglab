function [source] = ft_sourceanalysis_par(cfg, data, baseline)
...
elseif istimelock && any(strcmp(cfg.method, {'lcmv', 'sam', 'mne', 'harmony', 'rv', 'music', 'pcc', 'mvl', 'sloreta', 'eloreta'}))
  ...
  elseif strcmp(cfg.method, 'eloreta')
    %parallel eloreta addded by Ugo Bruzadin Nunes 04/12/2023
      haspar = [];
      haspar = ver('parallel');
      if isempty(haspar)
          for i=1:Nrepetitions
          ...
          % Preallocate the 'dip' variable
          dip = repmat(ft_datatype_dipole, 1, Nrepetitions);
          ...
          parfor i=1:Nrepetitions
              ...
              squeeze_Cy  = reshape(Cy(i,:,:), [size_Cy(2) size_Cy(3)]);
              fprintf('scanning repetition %d\n', i);
              dip(i) = ft_inverse_eloreta_par(sourcemodel, sens, headmodel, squeeze_avg, squeeze_Cy, methodopt{:}, leadfieldopt{:});
          end
      end
