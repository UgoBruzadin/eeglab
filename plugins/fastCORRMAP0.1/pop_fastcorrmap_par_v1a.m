...
%function EEG = pop_fastcorrmap_par_v1a(PATHTEMP,PATHOUT,numOfRejectionsPerTemplate,threshold)
function pop_fastcorrmap_par_v1a(EEG)

% define in and out folder

PATHTEMP = uigetdir(pwd,'CHOOSE TEMPLATE FOLDER');
PATHIN = uigetdir(pwd,'CHOOSE FOLDER WITH FILES');
PATHOUT = uigetdir(pwd,'CHOOSE FOLDER TO SAVE FILES');
...
%---- NEEDS TO BE FIXED if default etc
...
cd (PATHTEMP);
templates = dir('*.set') % contains all templates from the folder
cd (PATHOUT);
