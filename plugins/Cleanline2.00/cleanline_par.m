...
parEEG(1:length(g.chanlist(:)')) = deal(EEG);
gg(1:length(g.chanlist(:)')) = deal(g);
...
parfor ch = g.chanlist
    ...
    k = parEEG(ch).k
    Sorig = parEEG(ch).Sorig;
    Sclean = parEEG(ch).Sclean;
    ...
    parEEG(ch).k = k;
    parEEG(ch).Sorig = Sorig;
    parEEG(ch).Sclean = Sclean;
...
% --- redistribute parEEGs back to EEG
    for ch = g.chanlist
        Sorig(ch) = parEEG(ch).Sorig(ch);
        Sclean(ch) = parEEG(ch).Sclean(ch);
        if strcmpi(g.sigtype,'components')
            EEG.icadata(ch,:) = parEEG(ch).data(ch,:);
        else
            EEG.data(ch,:) = parEEG(ch).data(ch,:);
        end
    end
