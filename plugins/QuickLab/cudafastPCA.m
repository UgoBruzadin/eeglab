function EEG = cudafastPCA(EEG,IC)
if isempty(EEG.data)
    EEG = pop_loadset();
    eeglab redraw
end
...
if nargin < 2
    ...
    EEG = pop_par_runica(EEG,'extended', 1,'icatype','cudaica','verbose','off');
else
    if ~isempty(mybadcomps)
        fprintf('Rejecting selected components... \r');
        EEG = pop_subcomp(EEG, mybadcomps, 0);       % actually removes the flagged components
    end
    EEG = pop_par_runica(EEG,'extended', 1,'icatype','cudaica','pca',IC, 'verbose','off');
end
