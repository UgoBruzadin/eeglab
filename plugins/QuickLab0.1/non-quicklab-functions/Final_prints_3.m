...
Scale_V1 = 8; %Change the volt scale
EpochNum_v1a = 8; %Number of epoches display
XaxisofFig = 6000;
YaxisofFig = 4000;
...
for i=1:length(file1)
    ...
    % --- get iclabels, print, save
    EEG = pop_iclabel(EEG, 'default');
    IC = size(EEG.icaweights,1);
    [EEG,com] = pop_viewprops_adv(EEG, 0, [], 1:size(EEG.icawinv,2), {'freqrange',[2 55]},{});
    %pop_viewprops( EEG, 0,[1:IC], {'freqrange', [2 32]}, {}, 2, 'ICLabel' )
    saveas(gcf,[file1(i).name(1:end-4),'_ICA.jpg']);
    close gcf

    % ---  produce FFt and save
    quick_spectra(EEG,40,2,'AVG');
    saveas(gcf,[file1(i).name(1:end-4),'_FFT.jpg']);
    close gcf;
end
% 
% 
...
%     EEG = pop_loadset(file1(i).name, path1);
% 
%     pop_eegplot_v1( EEG, 0, 1, 1,[],'normed',1,'spacing',8,'winlength',8);
%     %pop_eegplot_adv(EEG, 2, 2, 1, 1,[],'normed',1,'spacing',8,'winlength',8)
%     normalize_chan(gcf);
%     normalize_chan(gcf);
%     %DeletefristPlot = get(gcf,'children');
%     %set(gcf, 'PaperUnits', 'inches', 'PaperPosition', [0 0 XaxisofFig YaxisofFig]/200);
%     set(gcf,'WindowState','maximized');
%     print(gcf, '-dpng', '-r200', strcat(file1(i).name(1:end-4),'_SCR.jpg'));
%     close all
% end



function com = pop_eegplot_v1( EEG, icacomp, superpose, reject, topcommand, varargin)
...
if icacomp == 1
    eegplot_v1a( EEG.data, 'srate', EEG.srate, 'title', ['Scroll channel activities -- eegplot() -- ', EEG.setname], ...
    ...
    eegplot_v1a( tmpdata, 'srate', EEG.srate, 'title', ['Scroll component activities -- eegplot() -- ', EEG.setname], ...
        'limits', [EEG.xmin EEG.xmax]*1000 , 'command', command, eegplotoptions{:}, varargin{:});
end
...
function [outvar1] = eegplot_v1a(data, varargin); % p1,p2,p3,p4,p5,p6,p7,p8,p9)
...
if ~ischar(data) % If NOT a 'noui' call or a callback from uicontrols
    ...
    try, g.spacing; 			catch, g.spacing	= Scale_V1; 	end
    try, g.eloc_file; 		catch, g.eloc_file	= 0; 	end; % 0 mean numbered
    g.winlength	= EpochNum_v1a;% Number of seconds of EEG displayed
    ...
    try, g.normed;            catch, g.normed = 1; end; %ozgur
    ...
    eegplot_v1a('scaleeye', [], gcf);
    if strcmp(lower(g.scale), 'off')
  	  eegplot_v1a('scaleeye', 'off', gcf);
    end

    % eegplot('drawp', 0);
    eegplot_v1a('drawp', 0);
    if g.dispchans ~= g.chans
   	   eegplot_v1a('zoom', gcf);
    end;
    eegplot_v1a('scaleeye', [], gcf);
...
%     			p2 =[];
%             end
%     		eegplot_v1a( 'drawp', p1, p2, g.children);
%     		figure(figh);
%          end
