...
%for iSelect = select(:)'
    ...
    EEGspread(i).lor.freq(freq) = freqPre;
    EEGspread(i).lor.source(freq) = sourcePost_nocon;
...
%     mri = ft_volumereslice([], mri.mri);
    ...
    EEGspread(i).lor.source_int(freq) = sourcePostInt_nocon;
...
for j=1:size(EEGspread,2)
    ft_sourceplot(cfg2,EEGspread(j).lor.sourcePostInt_nocon);
end
ft_sourceplot(cfg2,sourcePostInt_nocon);
%% history
disp('Done');
