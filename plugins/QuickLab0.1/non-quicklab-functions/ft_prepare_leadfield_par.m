function [sourcemodel, cfg] = ft_prepare_leadfield_par(cfg, data)
...
[headmodel, sens, cfg] = ft_prepare_headmodel(cfg, data);
...
elseif ft_headmodeltype(headmodel, 'interpolate')
  ...
  %Parallel function addded by Ugo Bruzadin Nunes 04/08/2023

haspar = [];
haspar = ver('parallel');

leadfield = cell(1,insideindx(end));
...
if ~isempty(haspar)
    parfor i=1:length(insideindx)
        ...
        %fprintf('computing leadfield %d\n in parallel', i)
        thisindx = insideindx(i);
        leadfield{i} = ft_compute_leadfield(sourcemodel.pos(thisindx,:), sens, headmodel, leadfieldopt{:});
...
%sourcemodel2 = sourcemodel; % for debugging purposes
for i=1:length(insideindx)
    %sourcemodel2.leadfield{insideindx(i)} = leadfield{i}; % for debugging purposes
    sourcemodel.leadfield{insideindx(i)} = leadfield{i};
end
