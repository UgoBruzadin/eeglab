...
function [EEG,com,EEGlor,elec,headmodel,vol,mri,fftdata,sourcemodel,grid,sourcemodel_int] = pop_dipfit_loretaQL(EEG, select, range, frequencies, varargin)
...
ftEEGdata = eeglab2fieldtrip(EEGOUT, 'preprocessing', 'none');
...
transform = [0.05476 -17.3653 -8.1318 0.075502 0.0031836 -1.5696 11.7138 12.7933 12.213];
%transform = [1         1       1      1+pi/2        1               1       10       10       10];
%transformmat = dat.transform;

elec1    = ftEEGdata.elec;
electransf.pnt = [];
...
ftEEGdata.elec.chanpos = electransf.pnt;
...
cfg.pad = 'nextpow2';
cfg.gpu = 'yes';
fftdata = ft_freqanalysis(cfg, ftEEGdata);
elec = fftdata.elec;
