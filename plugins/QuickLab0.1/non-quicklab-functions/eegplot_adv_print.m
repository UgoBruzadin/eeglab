% eegplot_adv_print() - Advanced Data Editor and Scroll (horizontally and/or vertically) through multichannel data.
...
%
% Usage: 
%           >> eegplot_adv_print(data, 1, 2, 1, 1, 'key1', value1 ...); % use interface buttons, etc.
%      else
%           >> eegplot_adv_print('noui', data, 1, 2, 1, 1, 'key1', value1 ...); % no user interface;
%                                                         % use for plotting
% Menu items:
%
...
%    'tag'        - [string] Matlab object tag to identify this eegplot_adv_print() window (allows 
%                    keeping track of several simultaneous eegplot_adv_print() windows). 
%    'children'   - [integer] Figure handle of a *dependent* eegplot_adv_print() window. Scrolling
...
function [outvar1,EEG] = eegplot_adv_print(EEG, varargin) % p1,p2,p3,p4,p5,p6,p7,p8,p9)
...
if ~ischar(data) % If NOT a 'noui' call or a callback from uicontrols
    ...
    %% NEW ADDED FEATURE, CLOSES OTHER eegplot_adv_print
    otherfigs = findobj('tag','eegplot_adv_print');
   ...
   try
       ...
       disp('eegplot_adv_print() error: calling convention {''key'', value, ... } error'); return;
   end
...
%        % Check  consistency of freqs
  ...
  % ---------------------------------
  defdowncom   = 'eegplot_adv_print(''defdowncom'',   gcbf);'; % push button: create/remove window
  defmotioncom = 'eegplot_adv_print(''defmotioncom'', gcbf);'; % motion button: move windows or display current position
  defupcom     = 'eegplot_adv_print(''defupcom'',     gcbf);';
  defctrldowncom = 'eegplot_adv_print(''topoplot'',   gcbf);'; % CTRL press and motion -> do nothing by default
  ...
  %% Try Defaults
   ...
   try g.title; 		    catch, g.title		= ['Scroll activity -- eegplot_adv_print()']; 	end
   ...
   try g.tag; 				catch, g.tag		= 'eegplot_adv_print'; end
   ...
   for index=1:length(gfields)
      ...
      otherwise, error(['eegplot_adv_print: unrecognized option: ''' gfields{index} '''' ]);
      end
   end
  ...
  if ischar(g.eloc_file) || isstruct(g.eloc_file)  % Read in electrode names
      ...
      eegplot_adv_print('setelect', g.eloc_file, ax1);
  end
...
%              new_menus(i) = copyobj(eeglab_menus.Children(i),findobj('tag','eegplot_adv_print'),'legacy');
...
% 
%     precallback = ['g = get(findobj(''tag'',''eegplot_adv_print''),''Userdata''); EEG = g.EEG; figure;'];
%     postcallback = ['fig = findobj(''tag'',''eegplot_adv_print''); g = get(fig,''Userdata''); g.EEG = EEG; set(fig,''Userdata'',g); eegplot_adv_print(''RESET'')'];
%     
...
% filelist
loadfilecommand = ['try cd(EEG.filepath); catch, end;files = dir(''*.set''); newEEG = pop_loadset(files(get(findobj(''tag'',''FolderList''),''value'')).name, pwd);com = pop_eegplot_adv_print(newEEG, 1, 2, 1, 1);[ALLEEG newEEG CURRENTSET] = eeg_store(ALLEEG, newEEG, CURRENTSET);'];
...
displayep = ['eegplot_adv_print(''redraw'')'];
togglerej = ['eegplot_adv_print(''rejection'')'];
displaycomp = ['EEG.plotEp = 1 - EEG.plotEp; eegplot_adv_print(EEG,varargin);'];
...
ICLcommand = ['g = get(gcf,''UserData''); [EEG,com] = quick_IClabel(g.EEG,[],[],[],[],[''eegplot_adv_print(''''MERGE_REJECTION'''');'']);'];
...
u(50) = uicontrol('Parent',figh,'Units', 'normalized','Position', posbut(44,:),	'Style','edit', ...
	...
	'Tag','SaveNowButton','string','Save File with Text','callback',['eegplot_adv_print(''SAVE'');']);
...
% tag
  ...
  u(40) = uicontrol('Parent',figh,'Units', 'normalized','Position', posbut(40,:), ...
    'Tag','TBT','BackgroundColor',DEFAULT_OFF_COLOR,'string','Run','Callback', 'eegplot_adv_print(''METHODS'');' );

  u(60) = uicontrol('Parent',figh,'Units', 'normalized','Position', posbut(32,:), ...
	'Tag','APPLY','BackgroundColor',DEFAULT_OFF_COLOR,'string','Apply Changes',	'Callback', ['eegplot_adv_print(''APPLY'');'] );

  %posbut(32
  u(41) = uicontrol('Parent',figh,'Units', 'normalized','Position', posbut(41,:),...
	'Tag','ClearMarks','BackgroundColor',DEFAULT_OFF_COLOR,'string','Clear Marks','Callback', ['eegplot_adv_print(''ClearMarks'');']);

%% Save, Tag and ICA attempts #Ugo 5/27/2022
% u(43) = uicontrol('Parent',figh,'Units', 'normalized','Position', posbut(43,:),
% 	'Tag','SaveTagICA', 'BackgroundColor',[.5 1 0.5],'string','Tag, Save & ICA','Callback', ['eegplot_adv_print(''saveandtag'')'] );
...
u(45) = uicontrol('Parent',figh,'Units', 'normalized','Position', posbut(51,:),...
	'Tag','SWITCH','BackgroundColor',cmodecolor,'string',cmode,'Callback', ['eegplot_adv_print(''SWITCH'')']);

% u(46) = uicontrol('Parent',figh,'Units', 'normalized','Position', posbut(50,:), ...
% 	'Tag','UNDO', 'BackgroundColor',[.7 .7 1],'string','Undo','Callback', ['eegplot_adv_print(''UNDO'')']);
% 
% u(47) = uicontrol('Parent',figh,'Units', 'normalized','Position', posbut(53,:),'Tag','REDO','BackgroundColor',[.7 .7 1],'string','Redo','Callback', ['eegplot_adv_print(''REDO'')']);
...
%% Text edit fields: NumChans
  ...
  u(49) = uicontrol('Parent',figh, ...
	...
	'Tag','NumChan','BackgroundColor',DEFAULT_PLOT_BACKGROUND,'string',num2str(g.chans),'Callback', 'eegplot_adv_print(''winelec_text'')' );
...
%% ESpacing buttons: + -
  ...
  u(61) = uicontrol('Parent',figh,'Units', 'normalized','Position',posbut(52,:), ...
	'Tag','Pushbutton6','string','Show All','FontSize',10,'Callback',['eegplot_adv_print(''winelec_auto'')']);
...
cb_envelope = ['g = get(gcbf,''userdata'');'...
    ...
    'eegplot_adv_print(''drawp'',0);clear g;'];
...
if isempty(g.command) 
 ...
 acceptandsavecommand = [savecommand acceptcommand];
  ...
  if ~isempty(g.events)
      u(17) = uicontrol('Parent',figh,'Units', 'normalized','Position',posbut(17,:), ...
                        'string', 'Events', 'callback', 'eegplot_adv_print(''drawlegend'', gcbf)');
  end
  ...
  uimenu('Parent',m(7),'Label','Edit figure', 'Callback', 'eegplot_adv_print(''noui'');');
  ...
  m(11) = uimenu('Parent',m(1),'Label','Data select/mark', 'tag', 'displaywin', ...
          ...
          uimenu('Parent',m(11),'Label','Hide marks','Callback', ...
            ...
            'set(gcbf, ''userdata'', g);' ...
            'eegplot_adv_print(''drawb''); clear g;'] )
  ...
  % Scale Eye %%%%%%%%%
  timestring = ['[OBJ1,FIG1] = gcbo;',...
	        'eegplot_adv_print(''scaleeye'',OBJ1,FIG1);',...
		'clear OBJ1 FIG1;'];
  m(7) = uimenu('Parent',m(1),'Label','Show scale','Callback',timestring);
  
  % Title %%%%%%%%%%%%
  uimenu('Parent',m(1),'Label','Title','Callback','eegplot_adv_print(''title'')')
  ...
  uimenu('Parent',m(2),'Label','Time range to display',...
      'Callback','eegplot_adv_print(''window'')')
  
  % Electrode window %%%%%%%%
  uimenu('Parent',m(2),'Label','Number of channels to display',...
      'Callback','eegplot_adv_print(''winelec'')')
  ...
  uimenu('Parent',m(6),'Label','Load .loc(s) file',...
      'Callback','eegplot_adv_print(''loadelect'');')
  
  % Zooms %%%%%%%%
 % if ismatlab && verLessThan('matlab','8.4.0')
 %     zm = uimenu('Parent',m(2),'Label','Zoom off/on');
 %     commandzoom = [ 'set(gcbf, ''WindowButtonDownFcn'', [ ''zoom(gcbf,''''down''''); eegplot_adv_print(''''zoom'''', gcbf, 1);'' ]);' ...
 ...
 % end
  ...
  uimenu('Parent',m(2),'Label', 'Help', 'callback', 'pophelp(''eegplot_adv_print'');'); %changed from figh to m(2) > settings

  % Events %%%%%%%%
  ev = uimenu('Parent',m(2),'Label','Events');
  comeventmaxstring   = [ 'tmpg = get(gcbf, ''userdata'');' ...
                'tmpg.plotevent = ''on'';' ...
                'set(gcbf, ''userdata'', tmpg); clear tmpg; eegplot_adv_print(''emaxstring'');']; % JavierLC      
  comeventleg  = [ 'eegplot_adv_print(''drawlegend'', gcbf);'];
  ...
  %%%%%%%%%%%%%%%%%%%
   ...
   mouse_motion_com = {'@eegplot_adv_print,''mouse_motion'';'};
  ...
  %set(figh, 'windowbuttonmotionfcn', EEG = eegplot_adv_print('mouse_motion'));
...
%  set(figh, 'windowbuttonupfcn', commandrelease);
  ...
  eegplot_adv_print('scaleeye', [], gcf);
  if strcmp(lower(g.scale), 'off')
	  eegplot_adv_print('scaleeye', 'off', gcf);
  end
  
  eegplot_adv_print('drawp', 0);
  if g.dispchans ~= g.chans
  	   eegplot_adv_print('zoom', gcf);
  end 
  eegplot_adv_print('scaleeye', [], gcf);
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  fig = findobj('tag','eegplot_adv_print');
  switch data
...
%     end
  ...
  case 'mouse_motion'
      ...
      figh = findobj('tag','eegplot_adv_print');
  ...
  case 'SAVE'
      ...
      %eeglab redraw;
      set(findobj('tag','eegplot_adv_print'),'UserData',g);
        load_directory(EEG,fig)
%       loaddircommand = ['findex = [1];try cd(EEG.filepath); catch, end; filecount = [1];files = dir(''*.set'');findex = find(strcmp({files.name}, EEG.filename));files = dir(''*.set'');set(findobj(''tag'',''FolderList''),''string'',{files(1:end).name},''value'',find(strcmp({files.name}, EEG.filename)));'];
%       eval(loaddircommand);
  ...
  case 'SWITCH'
    ...
    %change_scale([],[],fig,p1)
%    eegplot_adv_print('zoom', fig);
%    egplot_w3('updateslider', fig);
%    eegplot_adv_print('drawp',0);	
%    eegplot_adv_print('scaleeye', [], fig);
...
if ~isempty(eyeaxes)
   ...
   eegplot_adv_print('setelect');
   try eegplot_adv_print('updateslider', fig); catch; end
   eegplot_adv_print('drawp',0);	
   eegplot_adv_print('scaleeye', [], fig);
...
{ 'Number of channels to display:' } , 'Change number of channels to display', 1,  { num2str(g.dispchans) });
   ...
   eegplot_adv_print('updateslider', fig);
   eegplot_adv_print('drawp',0);	
   eegplot_adv_print('scaleeye', [], fig);
  ...
  case 'loadelect' % load channels
	...
	eegplot_adv_print('setelect',[ inputpath inputname ],AXH0);
	return;
  ...
  case 'noui'
      if ~isempty(varargin)
          eegplot_adv_print( varargin{:} ); fig = fig;
      else 
          fig = findobj('tag', 'eegplot_adv_print');
      end
   ...
   case 'zoom' % if zoom
      ...
      if exist('p2', 'var') == 1
          if ismatlab && verLessThan('matlab','8.4.0')
              set(gcbf, 'windowbuttondownfcn', [ 'zoom(gcbf,''down''); eegplot_adv_print(''zoom'', gcbf, 1);' ]);
          else
...
%     end
  ...
  case 'defdowncom'
   ...
   otherwise
      error(['Error - invalid eegplot_adv_print() parameter: ',data])
  end
...
%     end
    ...
    % channels in the eegplot_adv_print figure window.
...
if strcmp(get(fig, 'SelectionType'),'extend')
    ...
    if g.EEG.plotchannels
        ...
        eegplot_adv_print('topoplot', fig);
    end
    return;
end
...
%     show_mocap_for_eegplot_adv_print(g.winrej); 
...
if point_is_valid
    ...
    switch evnt.Key
        ...
        case {'insert'} %CHANGED UGO
            eegplot_adv_print('window');

        case {'tab'}
            eegplot_adv_print('winelec');
        ...
        case {'t'} % GO FORWARD
            eegplot_adv_print('TBT')

%         case {'y'}% TAG AND SAVE
%             eegplot_adv_print('SAVE')
...
%         g.gnumber = length(g.old);
        ...
        set(gcf,'UserData',g);
        eegplot_adv_print('setelect');
        eegplot_adv_print('winelec_auto');
        draw_matrix(g);
...
%% ALL METHODS START HERE
   ...
   switch method(1).Value
       ...
       case 2
           newcom = ['[EEG,com] = quick_IClabel(g.EEG,[],[],[],[],[''eegplot_adv_print(''''MERGE_REJECTION'''')'']);'];
       case 3
...
fig = findobj('tag','eegplot_adv_print');
set(fig,'Name',['Advanced EEG Data Editor by Ugo Bruzadin Nunes -- eegplot_adv_print(): ',EEG.filename]);
...
figh = findobj(gcf,'tag','eegplot_adv_print');
