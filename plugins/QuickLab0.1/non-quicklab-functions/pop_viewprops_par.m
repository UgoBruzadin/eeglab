...
function [EEG,com] = pop_viewprops2_par( EEG, typecomp, chanorcomp, spec_opt, erp_opt, scroll_event, classifier_name, fig, newcommand)
...
%         axis square;
        ...
        button(ri) = uicontrol(gcf, 'Style', 'pushbutton', 'Units','Normalized', 'Position',...
            [X(ri)  Y(ri) +sizewy sizewx/3 sizewy*0.18].*s+q, 'tag', ['comp' num2str(ri)]);
        ...
        checktag(ri)  = int2str(ri);
        
        % checkcom = ['pop_viewprops2_checkbox(' int2str(ri) ' )'];
        checkcom(ri)  = {@checkbox,int2str(ri)};

        check(ri)  = uicontrol(gcf, 'Style', 'checkbox','Units','Normalized','Tag',int2str(ri), 'Value',EEG.reject.gcompreject(ri),'Position',...
...
%                 
        ...
        ha(ri) = axes('Units','Normalized', 'Position',[X(ri) Y(ri) sizewx sizewy].*s+q,'Tag',strcat('H',int2str(ri)));
        if typecomp
...
%                     end
                     [prob, classind] = max(EEG.etc.ic_classification.(classifier_name).classifications(ri, :));
                     t(ri) = title(sprintf('%s : %.1f%%', ...
                         EEG.etc.ic_classification.(classifier_name).classes{classind}, ...
                         prob*100));
                     set(t(ri), 'Position', get(t(ri), 'Position') .* [1 -1.2 1])
%                 end
%             end
        end
        ...
        axis square;
                % --- edits to the topoplots
        topotag(ri) = strcat('T',checktag(ri));
        set(to(ri) ,'ButtonDownFcn', checkcom(ri));
        set(to(ri) ,'Tag', topotag(ri));
