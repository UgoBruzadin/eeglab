...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [OUTEEG, com] = pop_dipfit_settingsQL ( EEG, varargin )
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if nargin < 2
    ...
    if ischar(EEG)
        ...
        elseif isequal(EEG, 'coregister') % redraw
            ...
            % UGO added tmpelecs to be collected from the coregister function
...
if ischar(g.coord_transform)
    if isequal(g.coord_transform, 'warpfiducials')
        [tmpelecs,coord_transform] = coregister(EEG.chaninfo.nodatchans, OUTEEG.dipfit.chanfile, 'warp', 'auto', 'manual', 'off');
        OUTEEG.dipfit.coord_transform = coord_transform;
        OUTEEG.dipfit.newchanpos = tmpelecs;
    elseif isequal(g.coord_transform, 'alignfiducials')
        [tmpelecs,coord_transform] = coregister(EEG.chaninfo.nodatchans, OUTEEG.dipfit.chanfile, 'warp', 'auto', 'warpmethod', 'globalrescale', 'manual', 'off');
        OUTEEG.dipfit.coord_transform = coord_transform;
        OUTEEG.dipfit.newchanpos = tmpelecs;
    ...
    [OUTEEG.dipfit.newchanpos,coord_transform] = coregister(EEG.chanlocs, OUTEEG.dipfit.chanfile, 'transform',g.coord_transform,'manual','off');
    OUTEEG.dipfit.coord_transform = coord_transform;
end
