...
function EEG = eeg_compatlas3(EEG, varargin)
...
for iComp = g.components(:)'
    ...
    elseif size(EEG.dipfit.model(iComp).posxyz,1) == 2
        ...
        atlascoord2 = tfinv * [EEG.dipfit.model(iComp).posxyz(2,:) 1]';
        % find close location in Atlas
        distance = sqrt(sum((hm.cortex.vertices-repmat(atlascoord(1:3)', [size(hm.cortex.vertices,1) 1])).^2,2));
        distance2 = sqrt(sum((hm.cortex.vertices-repmat(atlascoord2(1:3)', [size(hm.cortex.vertices,1) 1])).^2,2));
        % compute distance to each brain area
        [~,selectedPt] = min( distance );
        [~,selectedPt2] = min( distance2 );
        area = hm.atlas.colorTable(selectedPt);
        area2 = hm.atlas.colorTable(selectedPt2);
        ...
        if area2 > 0
            EEG.dipfit.model(iComp).areadk2 = hm.atlas.label{area2};
        else
            EEG.dipfit.model(iComp).areadk2 = 'no area';
        end
        fprintf('Component %d: area %s\n', iComp, EEG.dipfit.model(iComp).areadk);
        fprintf('Component %d: area2 %s\n', iComp, EEG.dipfit.model(iComp).areadk2);
    else
