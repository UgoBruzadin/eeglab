...
function EEG = eeg_compatlasQL(EEG, source, varargin)
...
if nargin < 2 || isempty(source)
    error('You must run eLORETA source localization first');
end
...
[max_pow, max_idx] = max(source.avg.pow, [], 1); % find the maxima for each frequency
source_pos_mni = source.pos(max_idx, :); % extract the MNI coordinates of the maxima

fprintf('Looking up brain area in the Desikan-Killiany Atlas\n');
for iFreq = 1:size(source_pos_mni, 1)
    atlascoord = tfinv * [source_pos_mni(iFreq, :) 1]';
    ...
    if area > 0
        source.brain_area{iFreq} = hm.atlas.label{area};
    else
        source.brain_area{iFreq} = 'no area';
    end
    
    fprintf('Frequency %d: area %s\n', iFreq, source.brain_area{iFreq});
end
