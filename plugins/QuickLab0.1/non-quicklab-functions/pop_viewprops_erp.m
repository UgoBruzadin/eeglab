...
function [com] = pop_viewprops_erp( EEG, typecomp, chanorcomp, spec_opt, erp_opt, scroll_event, classifier_name, fig, erpcom)

projchan = [];

[projchan chanliststr] = pop_chansel( { EEG.chanlocs.labels } );
...
%             'default',  inistr, 'title', 'View many chan or comp. properties -- pop_viewprops_erp');
%     catch
%         result = [];
%     end
%     if size( result, 1 ) == 0
...
if length(chanorcomp) > PLOTPERFIG
    for index = 1:PLOTPERFIG:length(chanorcomp)
        pop_viewprops_erp(EEG, typecomp, chanorcomp(index:min(length(chanorcomp),index+PLOTPERFIG-1)), ...
            spec_opt, erp_opt, scroll_event, classifier_name);
    end;
    com = sprintf('pop_viewprops_erp( %s, %d, %s, %s, %s, %d, ''%s'' )', ...
...
% set up the figure
% -----------------
column = ceil(sqrt( length(chanorcomp) ))+2;
rows = ceil(length(chanorcomp)/column);
if ~exist('fig','var')
    figure('name', [ 'View ' fastif(typecomp,'channels','components') ' properties - pop_viewprops_erp() (dataset: ' EEG.filename ')'], 'tag', currentfigtag, ...
        'numbertitle', 'off', 'color', BACKCOLOR);
    set(gcf,'MenuBar', 'none');
    pos = get(gcf,'Position');
    if ~typecomp && isfield(EEG.etc, 'ic_classification')
        set(gcf,'Position', [pos(1) 00 1500/7*column 800/5*rows*1.5]);
    else
        set(gcf,'Position', [pos(1) 00 1500/7*column 800/5*rows]);
    end
    incx = 120;
    incy = 110;
    sizewx = 100/column;
...
for ri = chanorcomp
    if exist('fig','var')
        ...
        if isempty(button)
            error( 'pop_viewprops_erp(): figure does not contain the component button');
        end;
    else
        button = [];
    end;
