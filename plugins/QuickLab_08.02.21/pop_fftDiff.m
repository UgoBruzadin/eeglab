% % --- this script was orinally written by Matthew Gunn
% % and adapted by Ugo Bruzadin Nunes for the purpose of the QUickLab plugin
% 
% function pop_fftDiff()
clc
clear
...
if isfloat(filePre1), error('No files selected'),end
% Clean up return string if only one file; figment from 'multiselect'
%if ischar(filePre1), filePre1={filePre1}; end  % convert char string to cellstr
%if ischar(filePost2), filePost2={filePost2}; end  % convert char string to cellstr

%filePre1 = dir();
%filePost2 = dir(filePost2);
...
%for i=1:length(filePre1)
% For Pre Figure
EEG1 = pop_loadset( filePre1, path2);
close all
...
sgtitle(strcat( filePre1(1:10),' Pre or File1'));
cd (path2);
saveas(gcf,[filePre1 'Pre.jpg'])
cd (path1);
saveas(gcf,['Pre.jpg'])
close(gcf)

% For Pre Figure
EEG2 = pop_loadset(filePost2, path3);
close all
...
sgtitle(strcat( filePost2(1:10),' Post or File2'));
cd (path3);
saveas(gcf,[filePost2 'Post.jpg'])
cd (path1);
saveas(gcf,['Post.jpg'])
close(gcf)
...
FFT_Difference(xDETLA,FreqList,reallimits, AbsoluteRelativeMapColor, minfreqidx, maxfreqidx, EEGChanLocation)
sgtitle('Pre Minus Post or File1 Minus File2');
saveas(gcf,[ 'PreMinusPost.jpg'])
close(gcf)

% For Combining Images
out = imtile({strcat('Pre.jpg'), strcat('Post.jpg'), strcat('PreMinusPost.jpg')});
imshow(out);
sgtitle(ProcessComparing);
saveas(gcf,[filePre1(1:10),'_',filePost2(1:10),'_Pre_vs_POST.jpg']);
delete (strcat('Pre.jpg'), ('Post.jpg'), 'PreMinusPost.jpg');
