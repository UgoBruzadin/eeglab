    ...
    methods(Static)
        ...
        function batches(batches,batchFolder) %store all batches to be done
            for i=1:length(batches)
                files = dir('*.set');
                BatchName = char(strcat('Batch_',num2str(i)));
                [batchFolder] = pipeliner.createBatchFolders(batchFolder,files,BatchName);
                fprintf('generating bach folders');
                pipeliner.pipeIn(batchFolder,batches(i));
            end
        end
        ...
        function pipeIn(batchFolder,commands) %store the functions to be rolled in this batch
            ...
            for i=1:length(commands)
                ...
                pipeliner.Function(commands(i,1), cell2mat(commands(i,2)),batchFolder,folderCounter)
            end
        end
        
        function Function(type,content,path,counter) %magic function, runs the code asked!
            ...
            %tic;
            folderLetter = char(counter+64);
            folderNameDate = strcat(folderLetter,'-',char(type),'-',char(t));
            %fileCounter = 1; %counts how many file shave been run; useful for later
            [filePRE, filePOST] = pipeliner.createfolders(path,files,folderNameDate); %creates a folder for the pipeline
            cd(filePRE);
            ...
            parfor i=1:length(files)
                ...
                action = str2func(strcat(fname,char(type))); %this call a function inside this function with the name asked for!
                
                [EEG, acronym] = action(content, EEG); %this is where the function runs the asked code!
                
                acronym = '_';
                EEG = eeg_checkset(EEG);
                %[ALLEEG EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);
                
                [individualReport] = pipeliner.tempReport(files(i).name,char(type),num2str(content),EEG);
                %try 
                
                EEG = pop_saveset(EEG, 'filename', [strcat(files(i).name(1:end-4), '_', [acronym], '.set')], 'filepath',filePOST);
                ...
                %Report_Name = acronym;
                save(strcat(files(i).name(1:end-4),acronym,'_report.mat'),'individualReport'); %saves the table in .mat format
        
                %xlswrite(strcat(acronym,'.xlsx'),finalReport);
            ...
            for i=1:length(reports)
                if i==1
                    finalReport = load(reports(i).name);
                else
                    %newReport = load(reports(1).name);
                    finalReport = cat(1,finalReport,load(reports(1).name));
                end
                save(strcat(string(acronym),'-','finalReport.mat'),'finalReport');
            end
        ...
        function [EEG, acronym] = ica(content, EEG) %works
            %content = cell2mat(content);
            EEG = pop_runica(EEG,'extended', 1, 'pca', content, 'verbose','off');
            EEG = pop_iclabel(EEG,'default');
            acronym = char(strcat('IC',num2str(content)));
        end
        
        function [EEG, acronym] = iclabel(content,EEG) %iclabel({'all',.9},EEG)
            EEG = eeg_checkset(EEG);
            EEG = pop_iclabel(EEG,'default');
            EEG = eeg_checkset(EEG);
        ...
        function [EEG, acronym] = icloop(content, EEG)
        ...
        function [tempTable] = tempReport(filename,content,type,EEG)
        ...
        function [EEG, acronym] = filter(content,EEG) %should work
            ...
            acronym = char(strcat('FI',num2char(content(1)),'-',num2char(content(2))));
        end
        ...
        function [EEG, acronym] = rereference(content,EEG) %untested
            switch content
                case 'LE' | 'le' | 'Le'
        ...
        function [EEG, acronym] = headmodel(content,EEG) %untested
        ...
        function [EEG, acronym] = cleanline(content,EEG) %can do 50hz, 60hz, 70, and so on! %should work
            for i=1:length(content)
        ...
        function [EEG, acronym] = interpolate(content, EEG) %should works
            ...
            %EEG = pop_reref( EEG, [],'refloc',struct('labels',{'Cz'},'Y',{0},'X',{0},'Z',{8.7919},'sph_theta',{0},'sph_phi',{0},'sph_radius',{0},'theta',{0},'radius',{0},'type',{''},'ref',{'Cz'},'urchan',{[]},'datachan',{0}));
            acronym = 'INT';%gotta add the interpolated channels
        end
        ...
        function [EEG, acronym] = baseline(content, EEG) %baseline asks for two numbers %untested
