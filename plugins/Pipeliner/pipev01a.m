...
classdef pipev01a
    methods(Static)
        ...
        function Function(type,content) %magic function, runs the code asked!
            ...
            [filePRE, filePOST] = ugo.createfolders(files,type);
            cd(filePRE)
            for i=1:length(files)
                ...
                EEG = pop_saveset(EEG, 'filename', [files(i).name(1:end-4), '_',Acronym,'.set'], 'filepath',filePOST);
                [ALLEEG EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);
                %FinalTable = ugo.storeica(files(i),EEG);
                ALLEEG = pop_delset(ALLEEG, 1);
            end
            %ugo.savemat(FinalTable,type);
            ugo.txt(strcat('processing ', type, ' is over'))
        end
        ...
        function savemat(FinalTable, type) %untested
            save(strcat(type,'_report.mat'),'FinalTable'); %saves the table in .mat format
        end
        
        function savexls(FinalTable) %untested
            writecell(FinalTable,'AllFilesComponents_v2a.xls');
        end
        
        function icaloop(content,EEG) %not ready for testing
        ...
        function FinalTable = storeica(files, EEG) %untested
        ...
        function storemat(EEG) %untested
        ...
        function ica(content, EEG) %works
            EEG = pop_runica(EEG,'pca', content, 'extended', 1, 'verbose', 'off');
