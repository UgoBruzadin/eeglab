...
%         end
        ...
        function Function() %magic function, runs the code asked!
            ...
            for i=1:length(files)
                ...
                [temporaryTable] = pipeliner2.tempReport(fileCounter, files(i).name,content,scriptName,EEG,t);
        ...
        function [filePRE, filePOST] = createfolders(files, scriptName) %untested
            ...
            mkdir (scriptName)
            mkdir (scriptName,  'pre');
            fdtfiles = dir('*.fdt');
            %filePRE = strcat(basefolder,'\', type, '\pre');
            filePOST = strcat(basefolder,'\',scriptName);
        ...
        function [tempTable] = tempReport(fileCounter,filename,scriptName,EEG)
            ...
            tempTable = cat(1,tempTable,{filename,scriptName,EEG.nbchan,EEG.chanlocs(1).ref,EEG.srate,EEG.trials,length(EEG.event),EEG.xmax,length(EEG.icaweights)});
            if ~isempty(EEG.icaweights)
