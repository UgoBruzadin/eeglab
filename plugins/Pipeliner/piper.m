    ...
    methods(Static)
        ...
        function Function(type,content) %magic function, runs the code asked!
            ...
            for i=1:length(files)
                ...
                %[acronym] = pipeliner.getAcronym(type,content);
                Acronym = type(1:2);
                EEG = pop_saveset(EEG, 'filename', [strcat(files(i).name(1:end-4), '_', Acronym, '.set')], 'filepath',filePOST);
                [ALLEEG EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);
                [temporaryTable] = pipeliner.tempReport(fileCounter, files(i),action,type,EEG);
                finalReport = cat(1,finalReport,temporaryTable);
                Report_Name = strcat(type,content);
                xlswrite(Report_Name,finalReport); 
                ALLEEG = pop_delset(ALLEEG, 1);
            end
            %pipeliner.savemat(FinalTable,type);
            pipeliner.txt(strcat('processing of ',string(action), ' is over'))
        end
        
        %functions without EEG
        
        function [acronym] = getAcronym(type,content)
        ...
        function txt(content) %works
            ...
            email = 'pipelinerbnunes@hotmail.com';
            who = {number, email};
            mail = 'pipelinerslab@gmail.com'; %Your GMail email address
        ...
        function [tempTable] = tempReport(fileCounter,filename,action,type,EEG)
            %pipeliner.tempReport(fileCounter, files(i),action,type,EEG)
            ExcelSheet = [];
            if fileCounter == 1
                ExcelSheetHeader = {'name','process','process info','numchans','ref','srate','trials','events','xmax','components','filesize'};
            ...
            tempTable = {filename,action,strcat(type),EEG.nbchan,EEG.chanlocs(1).ref,EEG.srate,EEG.trials,length(EEG.event),EEG.xmax};
            if ~isempty(EEG.icaweights)
